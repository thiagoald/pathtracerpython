...
def intersect(ray, triangle):
    ...
    v_plane = np.cross((np.array(v1)-np.array(v2)), (np.array(v3)-np.array(v2))) #get director vector of plane
    v_plane = v_plane/np.linalg.norm(v_plane) #normalize vector of plane
    dot = np.dot(vr, v_plane) #check if they're not collinear (line parallel to plane)
    if (abs(dot)>1e-5):
        t = (np.dot(v_plane, np.array(v1)) - np.dot(v_plane, np.array(pr)))/np.dot(v_plane, vr) #calculate intersection parameter
